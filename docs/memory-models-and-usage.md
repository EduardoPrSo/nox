...
- metadata normalizada de uso: identidade, provider, modelo, capability, tokens, latência e custo quando informado.

Não são persistidos headers HTTP, Bearer tokens, API keys, connection strings nem a resposta bruta do OpenRouter. Credenciais não devem ser enviadas no texto da conversa. Áudio, imagens, embeddings e transcrições ambientes não são armazenados neste milestone.
...
Fallbacks atuais são finitos: `FAST → DEFAULT`, `REASONING → DEFAULT`, `CODING → REASONING → DEFAULT` e `MEMORY → FAST → DEFAULT`. `VISION`, `STT` e `TTS` exigem configuração explícita. Não existe escalonamento recursivo ou automático neste milestone.

## AI usage e budget

Uma linha é gravada por chamada ao provider. O custo usa decimal PostgreSQL (`numeric(24,12)`), nunca ponto flutuante no domínio. Quando o provider não informa um campo, ele fica `null`. Falha de telemetria é registrada no logger da aplicação e não altera uma resposta válida.
